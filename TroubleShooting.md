## 🏷️ 몬스터 라벨 및 이름 표시 문제 (2024-12-19)

### 문제 상황
- 오크 군락지와 라벨이 겹쳐서 보기 어려움
- 오크 생성 로그는 나오지만 실제 유닛 이름이 "고블린"으로 표시됨

### 해결 방법
1. **몬스터 라벨 이름 표시 개선**
   - `EntityRenderer.jsx`에서 하드코딩된 "고블린" 라벨을 `m.name`으로 변경
   - 실제 몬스터 객체의 이름 속성을 사용하도록 수정

2. **오크 라벨 위치 조정**
   - 오크는 크기가 더 크므로(`size: 1.2`) 라벨 높이를 `2.0*monsterSize`로 조정
   - 고블린은 기존대로 `1.5*monsterSize` 유지
   - 몬스터 타입에 따라 다른 라벨 높이 적용

3. **디버깅 로그 개선**
   - `monsterNestManager.js`에서 생성된 몬스터의 실제 이름을 로그에 포함
   - 생산 과정에서 올바른 이름 확인 가능
...
### 문제 상황
- HUD의 건물목록에서 적 팀 건물이 표시됨
- 각 몬스터 군락에서 올바른 몬스터가 스폰되지 않는 문제 의심

### 해결 방법
1. **HUD 건물목록 필터링**
   - `MainMenu.jsx`의 `renderBuildingsTab` 함수에서 `building.team === 0` 필터 추가
   - 플레이어 팀(팀 0) 건물만 표시하도록 수정

2. **몬스터 스폰 디버깅**
   - `monsterNestManager.js`의 `produceMonster` 함수에 디버깅 로그 추가
   - `state.js`의 초기화 과정에 로그 추가
   - 각 군락지 타입과 생성되는 몬스터 타입 확인

### 결과
- HUD에서 적 팀 건물이 숨겨짐
- 각 몬스터 군락에서 올바른 몬스터가 스폰되는 것을 확인
- 고블린 군락지(`goblin_den`)에서 고블린, 오크 군락지(`orc_den`)에서 오크 생산 확인
