...
#### 2. 색상 생성과 적용 분리
- **색상 생성**: `getOrCreateCitizenColor(citizenId, appearance)` - 한 번만 실행
- **색상 적용**: `applyCitizenColor(citizenId, instanceIndex)` - 매 프레임 실행
- **구현**:
  ```javascript
...
#### 3. 완전한 색상 독립성 보장
- **시민 생성 시**: ID 기반으로 고유 색상 계산하여 Map에 저장
- **렌더링 시**: 저장된 색상을 현재 인덱스에 적용
- **건물 출입 시**: 다른 시민의 인덱스가 변해도 각자의 고유 색상 유지

#### 4. 몬스터도 동일한 시스템 적용
- 몬스터도 ID 기반 색상 시스템 사용
- `getOrCreateMonsterColor`, `applyMonsterColor` 함수 구현

### 🎯 결과
- ✅ **시민 ID 기반 완전한 색상 고정**: 절대 변하지 않는 고유 색상
- ✅ **건물 출입 시 색상 안정성**: 다른 시민들의 색상이 절대 변하지 않음
- ✅ **색상과 위치 완전 분리**: 색상은 ID 기반, 위치는 렌더링 기반
- ✅ **단순하고 확실한 시스템**: 각 시민이 자신만의 색상을 영구 소유

### 🐛 버그 수정
- **에러 해결**: `citizenIdToIndex is not defined` 참조 오류 수정
- **원인**: 제거된 변수 `citizenIdToIndex`를 코드에서 여전히 사용
- **해결**: 불필요한 인덱스 매핑 코드 완전 제거

---

## 2025-01-27 - 시민 배정 해제 시 이동 중단 및 UI 정리

### 📋 작업 요약
시민 배정 해제 시 이동 중인 시민을 멈추는 기능과 UI에서 몬스터 관련 요소들을 제거했습니다.

### 🔧 주요 변경사항

#### 1. 시민 배정 해제 시 이동 중단 기능
- **파일**: `src/game/state.js`
- **문제**: 시민을 부르는 중 배정 해제하면 시민이 도착 후 목록에서 사라짐
- **해결**: `unassignUnit` 함수에 이동 중단 로직 추가
- **구현**:
  ```javascript
...
#### 2. HUD 건축 메뉴에서 몬스터 군락 제거
- **파일**: `src/components/BuildMenu.jsx`, `src/components/MainMenu.jsx`
- **변경**: 고블린 군락(`goblin_den`), 오크 군락(`orc_den`) 건축 옵션 제거
- **구현**:
  ```javascript
  // 기존: filter(t=>t!=="monster_den")
  // 변경: filter(t=>!["monster_den", "goblin_den", "orc_den"].includes(t))
  ```

#### 3. 시민 목록에서 몬스터 제거
- **파일**: `src/components/MainMenu.jsx`
- **변경**: 시민 탭에서 플레이어 팀(team 0)만 표시
- **구현**:
  ```javascript
...
### 🎯 결과
- ✅ **시민 배정 해제 시 이동 중단**: 부르는 중 해제해도 시민이 안전하게 정지
- ✅ **깔끔한 건축 메뉴**: 플레이어가 실제로 건설할 수 있는 건물만 표시
- ✅ **정리된 시민 목록**: 몬스터 없이 실제 시민만 표시
- ✅ **일관된 플레이어 경험**: 플레이어 관련 요소만 UI에 노출

---

## 2025-01-27 - 오크 군락 라벨 표기 수정

### 📋 작업 요약
오크 군락의 라벨이 "고블린 군락"으로 잘못 표시되는 문제를 해결했습니다.

### 🔧 주요 변경사항

#### 1. 가상 시민 이름 동적 설정
- **파일**: `src/game/systems/monsterNestManager.js`
- **문제**: 가상 시민(virtual worker)의 이름이 항상 "고블린 군락"으로 하드코딩됨
- **해결**: 군락지 타입(`nestType`)에 따라 동적으로 이름 설정
- **구현**:
  ```javascript
  // 기존: name: '고블린 군락'
  // 변경: name: nestType === 'orc_den' ? '오크 군락' : '고블린 군락'
  ```

#### 2. 매개변수 전달 로직 개선
- **변경**: `createVirtualWorker` 함수에 `nestType` 매개변수 추가
- **구현**: `createVirtualWorker(nestId, position, team, nestType)`
- **효과**: 군락지 생성 시 타입 정보가 가상 시민 이름에 정확히 반영

### 🎯 결과
- ✅ **오크 군락 라벨 정확 표시**: "오크 군락"으로 올바르게 표시
- ✅ **고블린 군락 라벨 유지**: "고블린 군락"으로 기존대로 표시
- ✅ **동적 라벨 시스템**: 새로운 군락지 타입 추가 시 확장 용이
- ✅ **일관된 표기**: 건물 타입과 라벨 표기가 정확히 일치
