...
- **시스템**: 생산/세금/명성/건설/건물XP/간이 스폰(명성+1)/몬스터 시스템
  - 밸런스 조정(2025-09-16):
  ...
  - 시민 데이터 구조: 스탯(`STR/AGI/VIT/INT`), 스킬(재능, `Farming~Admin` — 모든 스킬 최소 1), 수련치(practice: 0~10), 인벤토리, 외형, 초기외형(originalAppearance)
  - 시민 무작위 생성 유틸: 이름/외형/스탯/스킬(재능)/수련치를 부여해 스폰 시 자동 적용
  - 시민 외형 고정 시스템: 건물에 숨겨진 상태에서도 초기 외형 유지
  - **새로운 생산 시스템**: 시간 기반 생산, 시민 숨김 처리, 생산 진행률 표시, 재능에 따른 확률적 수련치 상승
  - **자원 표시 시스템**: 건물 생산 완료 시 자원 이모지와 생산량이 건물 위에 올라가면서 사라지는 애니메이션
  - **몬스터 시스템**: 완전 통합 시스템, 몬스터와 시민이 동일한 렌더러/애니메이션/이동 시스템 사용, 팀 기반 소속 구분, 몬스터 생산 건물, 자동 군락지 설치, 랜덤 스탯 고블린/오크 생산, 군락지 주변 배회 시스템, 자동 생산 및 활동성 향상, 모듈화된 군락지 관리 시스템, 다중 몬스터 타입 지원
- **HUD**: 상단 좌측 자원/인구/명성 표시(스토어 구독 기반 실시간 갱신)
- **3D 렌더**: InstancedMesh, 보행/스윙, 지형/바이옴, 상태 구동형 `props.units` 반영
- **MainMenu**: 우상단에 건축🏗️, 건물🏠, 시민👥, 부대⚔️, 창고📦, 제작🔨, 외교🤝 버튼으로 구성된 탭 메뉴 시스템
  - 건축 탭: 건물 목록 및 건설 비용 표시, 클릭 시 청사진 기반 건설 모드 진입
  - 건물 탭: 현재 건물 목록 표시, 클릭 시 인스펙터에서 건물 정보 확인
  - 시민 탭: 현재 시민 목록 표시, 클릭 시 인스펙터에서 시민 정보 확인
  - 부대 탭: 부대 목록 표시, 명령/배치 버튼으로 부대 관리, 배치 시 시민 소속 관리 가능
...
- **선택/인스펙터**: 건물, 시민, 아이템, 부대를 클릭해 선택, 하단 인스펙터 표시(더블클릭 포커스 이동)
  - 시민 인스펙터: 가로 3단(기본정보: 직업(속한 건물)/HP(현재/최대)/MP(현재/최대)/스탯/전투스탯 · 스킬(재능)/수련치 · 인벤토리)
  - 아이템 인스펙터: 아이템 정보, 이름 변경, 사용/장착 버튼, 제작 비용 표시
  - 부대 인스펙터: 부대 이름 변경, 시민 소속 관리, 부대 이동 모드 활성화
- **부대 시스템**: 시민들을 부대로 편성하여 집단 이동 및 관리
...
│   │   ├── MainMenu.jsx          # 우상단 탭 메뉴 시스템 (건축/건물/시민/부대/창고/제작/외교)
│   │   ├── HUD.jsx               # 상단 좌측 자원/인구/명성 표시
│   │   ├── Inspector.jsx        # 하단 중앙 선택된 건물/시민/아이템/부대 정보 표시
│   │   ├── SquadInspector.jsx   # 부대 인스펙터 컴포넌트
│   │   ├── SquadMoveDisplay.jsx # 부대 이동 모드 시 HP/MP 표시 컴포넌트
...
│       ├── townRangeCalculator.js
│       └── combatUtils.js         # 전투 유틸리티 함수들
│
│   └── game/
...
│       │   ├── goblins.js              # 고블린 정의 및 생성 함수
│       │   ├── orcs.js                 # 오크 정의 및 생성 함수
...
│           ├── monsterNestManager.js # 몬스터 군락지 관리 시스템
│           ├── squadAI.js           # 부대 AI 시스템
│           └── combatUtils.js       # 전투 유틸리티 함수들
...
### 모바일 화면 비율 처리(가로 강제)
- 세로 화면이거나 가로가 더 짧은 경우, 앱이 자동으로 가로로 회전되도록 단순 처리합니다.
- 동작 방식: 런타임에서 `window.innerWidth < window.innerHeight`면 `App` 루트에 `.force-landscape` 클래스를 토글합니다.
- CSS: `.force-landscape`는 앱 컨테이너만 90도 회전하며, 문서 전체 회전이나 복잡한 미디어쿼리는 사용하지 않습니다.
- 장점: 흰 화면/빈 화면 문제 없이 단순하고 일관된 동작.
...
### 배포 설정
- **Vite 설정**: `base: '/3D_Poly_Dynasty/'`로 설정되어 GitHub Pages의 서브디렉토리 배포에 최적화
- **자동 배포**: main 브랜치에 푸시할 때마다 자동으로 gh-pages 브랜치에 배포
- **배포 URL**: `https://szara7678.github.io/3D_Poly_Dynasty/`
...
### 배포 확인
- `https://szara7678.github.io/3D_Poly_Dynasty/`에서 사이트 접속
